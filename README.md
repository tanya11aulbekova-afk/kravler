...
Вывод цветной и аккуратный (баннер, подсветка совпадений, статусы); при первом
запуске конфиги (`keywords.txt`, `patterns.txt`, `urls_seed.txt`, `proxies.txt`)
создаются рядом автоматически.

## Запуск через .exe (Windows, без Python)

Нужен один файл `TelegraphParser.exe`, запускается двойным кликом — откроется меню.
Собери exe одним из способов:

**Локально:** двойной клик по `build.bat` (нужен установленный Python). Готовый
файл появится в `dist\TelegraphParser.exe`. Скопируй его куда угодно и запускай —
конфиги и база (`articles_db.json`) создадутся рядом с exe сами.

**Через GitHub (Python не нужен вообще):** во вкладке **Actions → build-exe →
Run workflow**. Когда сборка закончится, скачай `TelegraphParser-windows` из
раздела Artifacts — внутри готовый `TelegraphParser.exe`.

## Установка (запуск из исходника)
